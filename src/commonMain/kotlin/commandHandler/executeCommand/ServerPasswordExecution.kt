...
fun Session.serverPasswordExecution(command: ServerPasswordCommand) = section {
    ...
    if (settingsManager.settings.serverPassword == "") {
        if (command.getOldPassword() != null) {
            textLine("No previous password found. New password not saved.")
            return@section
        }
    } else {
        if (command.getOldPassword() == null) {
            textLine("Please provide current (old) password to confirm changes.")
            return@section
        }

        if (settingsManager.settings.serverPassword != command.getOldPassword()) {
            textLine("Old password was incorrect, please try again.")
            return@section
        }
    }

    settingsManager.setServerPassword(command.getNewPassword())
    textLine("Password saved. You will need to provide this new password if you want to change the default password again.")
}.run()
