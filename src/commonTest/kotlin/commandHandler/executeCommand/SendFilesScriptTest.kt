...
private class SendFilesScriptTest : BaseSocketScriptTest() {
    ...
    fun sendFilesNormal() = runSocketScriptTest2(
        setupBlock = {
            ...
            every { fileNamesTooLong(any(), any()) } returns FileTransfer.NORMAL.toName
    ...
    fun sendFilesCancel() = runSocketScriptTest2(
        setupBlock = {
            mockkStatic(::setupSendCommandClient2)
            every { setupSendCommandClient2(any()) } returns generateClient()
            every { fileNamesTooLong(any(), any()) } returns FileTransfer.CANCEL.toName
            every { sendFilesCollecting(any(), any()) } returns mockSourceFileListManager()
        },
        {
            sendFilesExecution(
    ...
    fun sendFilesSkipInvalid() = runSocketScriptTest2(
        setupBlock = {
            mockkStatic(::setupSendCommandClient2)
            every { setupSendCommandClient2(any()) } returns generateClient()
            every { fileNamesTooLong(any(), any()) } returns FileTransfer.SKIP_INVALID.toName
    ...
    fun sendFilesSkipCompressEverything() = runSocketScriptTest2(
        setupBlock = {
            mockkStatic(::setupSendCommandClient2)
            every { setupSendCommandClient2(any()) } returns generateClient()
            every { fileNamesTooLong(any(), any()) } returns FileTransfer.COMPRESS_EVERYTHING.toName
            every { sendFilesCollecting(any(), any()) } returns mockSourceFileListManager(length = 1)
        },
        {
            sendFilesExecution(
    ...
    fun sendFilesSkipCompressInvalid() = runSocketScriptTest2(
        setupBlock = {
            mockkStatic(::setupSendCommandClient2)
            every { setupSendCommandClient2(any()) } returns generateClient()
            every { fileNamesTooLong(any(), any()) } returns FileTransfer.COMPRESS_INVALID.toName
            every { sendFilesCollecting(any(), any()) } returns mockSourceFileListManager(length = 1)
        },
        {
            sendFilesExecution(
