fun encodeIpAddress(ipAddress: String): String {
    try {
        val hexes = ipAddress
            .split('.')
            .also {
                require(it.size == 4) { "Invalid IP address format" }
            }.map {
                it.toInt().also { num ->
        ...
        return "${getWord(firstWord)}-${getWord(secondWord)}-${hexes.last()}"
    } catch (e: Exception) {
...
fun decodeIpAddress(ipAddress: String): String {
    try {
        ...
        val addressChunk4 = tokens[2].toInt(radix = 16).also { require(it in (0..255)) }
